...
Car* getInfo(int size)
{
    ...
    int i;
    for(i=0;i<size;i++){
        printf("Enter the vehicle make(brand): ");//make
        scanf("%s", (inventory+i)->make);
        printf("Enter the vehicle model: ");//model
        scanf("%s", (inventory+i)->model);
        printf("Enter the vehicle price: ");//price
        scanf("%f", &(inventory+i)->price);
        printf("Enter the vehicle mileage: ");//miles
...
void printInfo(int size, Car* inventory)
{
    int i;
    for (i=0; i<size;i++){
        printf("\n\nMake: %s\nModel: %s\nMiles: %d\nPrice: $%.2f\n",(inventory+i)->make,(inventory+i)->model,(inventory+i)->miles, (inventory+i)->price);
    }//end of for
}//end of printinfo
